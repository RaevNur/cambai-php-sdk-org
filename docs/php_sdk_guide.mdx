...
description: "Get started with the Camb.ai PHP SDK in minutes"
icon: "php"
---
...
$payload->setSpeechModel(CreateStreamTTSRequestPayload::SPEECH_MODEL_MARS_8_FLASH);
...
<Tabs>
  ...
  {/* <Tab title="MARS Pro">
    ```php
    $payload->setSpeechModel(CreateStreamTTSRequestPayload::SPEECH_MODEL_MARS_8_PRO);
    $stream = $client->tts()->ttsTtsStreamPost($payload);
    ```
  ...
  <Tab title="MARS Instruct">
    ```php
    $payload->setSpeechModel(CreateStreamTTSRequestPayload::SPEECH_MODEL_MARS_8_INSTRUCT);
